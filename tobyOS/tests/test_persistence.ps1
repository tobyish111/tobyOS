...
function Start-Tobyos($logSuffix) {
    ...
    $serialF = Join-Path $LogDir "serial.$logSuffix.log"
    $debugF  = Join-Path $LogDir "debug.$logSuffix.log"
    $qemuF   = Join-Path $LogDir "qemu.$logSuffix.log"
    $qemuStd = Join-Path $LogDir "qemu.$logSuffix.stderr.log"
    Remove-Item -ErrorAction SilentlyContinue $serialF, $debugF, $qemuF, $qemuStd
    $args = @(
        "-cdrom", $iso,
        "-drive", "file=$disk,format=raw,if=ide,index=0,media=disk",
        "-smp", "4",
        "-serial", ("file:" + $serialF),
        "-debugcon", ("file:" + $debugF),
        "-d", "cpu_reset,guest_errors", "-D", $qemuF,
        "-no-reboot", "-no-shutdown",
        "-display", "none",
        "-qmp", "tcp:127.0.0.1:4445,server,nowait"
    )
    $p = Start-Process -FilePath $qemu -ArgumentList $args -PassThru -RedirectStandardError $qemuStd
...
Get-Content (Join-Path $LogDir "debug.pass1.log") -ErrorAction SilentlyContinue -Tail 80
...
Get-Content (Join-Path $LogDir "debug.pass2.log") -ErrorAction SilentlyContinue -Tail 80
