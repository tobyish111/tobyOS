...
if ($ok1 -and $ok2) {
    ...
    Get-Content (Join-Path $LogDir "serial.m27f_vgpu.log") -Tail 20
    Write-Host ""
    Write-Host "Last 8 lines of serial.m27f_fb.log:"
    Get-Content (Join-Path $LogDir "serial.m27f_fb.log") -Tail 8
    exit 0
} else {
    ...
    if (-not $ok1) {
        ...
        Get-Content (Join-Path $LogDir "serial.m27f_vgpu.log") -Tail 80
    }
    if (-not $ok2) {
        Write-Host ""
        Write-Host "=== last 80 lines of serial.m27f_fb.log ===" -ForegroundColor Yellow
        Get-Content (Join-Path $LogDir "serial.m27f_fb.log") -Tail 80
    }
    exit 1
}
