...
#
# Usage (from tobyOS repo root):
#   pwsh tests/test_m27_final.ps1            # run all phases + summary
#   pwsh tests/test_m27_final.ps1 -Skip M27F # skip a phase by tag
#   pwsh tests/test_m27_final.ps1 -Build     # `make` first to ensure ISO is fresh
#
...
$phases = @(
    [PSCustomObject]@{ Tag='M27A'; Script=(Join-Path $PSScriptRoot 'test_m27a.ps1');
                       Desc='Display test harness (displayinfo / drawtest / rendertest / fonttest)' },
    [PSCustomObject]@{ Tag='M27B'; Script=(Join-Path $PSScriptRoot 'test_m27b.ps1');
                       Desc='Rendering backend cleanup (gfx_backend abstraction + bounds checking)' },
    [PSCustomObject]@{ Tag='M27C'; Script=(Join-Path $PSScriptRoot 'test_m27c.ps1');
                       Desc='Alpha blending + compositing (ARGB surfaces, source-over)' },
    [PSCustomObject]@{ Tag='M27D'; Script=(Join-Path $PSScriptRoot 'test_m27d.ps1');
                       Desc='Improved font rendering (supersampling AA, scaled bitmap)' },
    [PSCustomObject]@{ Tag='M27E'; Script=(Join-Path $PSScriptRoot 'test_m27e.ps1');
                       Desc='Dirty rectangles + redraw optimization (gfx_present_stats)' },
    [PSCustomObject]@{ Tag='M27F'; Script=(Join-Path $PSScriptRoot 'test_m27f.ps1');
                       Desc='virtio-gpu support (probe/init/2D scanout/present_rect + Limine fallback)' },
    [PSCustomObject]@{ Tag='M27G'; Script=(Join-Path $PSScriptRoot 'test_m27g.ps1');
...
$globalResults = @()
$serialA = Join-Path $LogDir "serial.m27a.log"
...
$forbiddenHits = @()
$logs = Get-ChildItem -Path $LogDir -Filter 'serial.m27*.log' -ErrorAction SilentlyContinue
foreach ($lf in $logs) {
...
Write-Host "    pwsh tests/test_m27a.ps1     # display test harness"
Write-Host "    pwsh tests/test_m27b.ps1     # rendering backend cleanup"
Write-Host "    pwsh tests/test_m27c.ps1     # alpha blending"
Write-Host "    pwsh tests/test_m27d.ps1     # font rendering"
Write-Host "    pwsh tests/test_m27e.ps1     # dirty rectangles"
Write-Host "    pwsh tests/test_m27f.ps1     # virtio-gpu support (TWO boots: with + without -device virtio-gpu-pci)"
Write-Host "    pwsh tests/test_m27g.ps1     # multi-monitor groundwork"
Write-Host ""
Write-Host "  Or run this aggregator:"
Write-Host "    pwsh tests/test_m27_final.ps1            # all phases + global sweep"
Write-Host "    pwsh tests/test_m27_final.ps1 -Build     # rebuild kernel ISO first"
Write-Host "    pwsh tests/test_m27_final.ps1 -Skip M27F # skip a phase by tag"
Write-Host ""
...
Write-Host '        -serial file:logs/serial.log -no-reboot'
Write-Host ""
