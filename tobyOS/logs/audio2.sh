...
WAV=logs/audio2.wav
WAV_WIN='C:/CustomOS/tobyOS/logs/audio2.wav'
LOG=logs/audio2.log
PORT=4456
...
if ! make "CC=TMP='C:\\t' TEMP='C:\\t' clang" \
     ...
     >logs/audio2.build.log 2>&1; then
    echo "ISO BUILD FAIL -- tail:"; tail -30 logs/audio2.build.log; exit 1
fi
...
echo
echo "=== audioplay ==="
grep -aE 'audioplay' "$LOG" | head -8
echo
echo "=== userland tool totals ==="
grep -a "M26A: userland" "$LOG" | tail -1
echo
