...
#include <tobyos/tls13.h>  /* transport-independent key schedule + AEAD */
...
/* The TLS 1.3 key schedule (HKDF, Derive-Secret, the handshake/app
 * traffic keys, Finished) and the IETF ChaCha20-Poly1305 AEAD now
 * live in tls13.c (transport-independent core shared with the future
 * QUIC crypto layer); see tls13.h. The wire helpers below are the
 * short aliases this file has always used. */
#define put_u16 tls_put_u16
#define put_u24 tls_put_u24
#define get_u16 tls_get_u16
#define get_u24 tls_get_u24
...
/* derive_handshake_keys / derive_app_keys / compute_finished are the
 * TLS 1.3 key schedule -- now in tls13.c (shared with QUIC). */
...
/* Chain validation + CertificateVerify verification now live in
 * tls_x509.c (transport-independent, shared with QUIC); tls.c calls
 * the tls_x509_* API. The short names below keep the call sites in
 * tls_do_handshake unchanged. */
#define tls_validate_chain(msg, msglen, host, pk, buf, cap) \
        tls_x509_validate_chain((msg), (msglen), (host), (pk), (buf), (cap))
#define tls_verify_cv(scheme, sig, sl, th, pk) \
        tls_x509_verify_cv((scheme), (sig), (sl), (th), (pk))
