#!/bin/bash
# Build the xattr + fd-identity + fexecve test (linux-stat.elf).
#
...
#
# Usage:   bash programs/linux-stat/build.sh
# Output:  programs/linux-stat/linux-stat.elf   (gitignored; opt-in)
set -e

HERE="$(cd "$(dirname "$0")" && pwd)"
OUT="$HERE/linux-stat.elf"
...
clang --target=x86_64-unknown-linux-gnu -static -O2 -g0 -fno-stack-protector \
  ...
  -c "$HERE/main.c" -o "$HERE/linux-stat.o"
