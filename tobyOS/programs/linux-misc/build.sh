#!/bin/bash
# Build the syscall-tail acceptance test (linux-misc.elf).
#
...
#
# Usage:   bash programs/linux-misc/build.sh
# Output:  programs/linux-misc/linux-misc.elf   (gitignored; opt-in)
set -e

HERE="$(cd "$(dirname "$0")" && pwd)"
OUT="$HERE/linux-misc.elf"
...
clang --target=x86_64-unknown-linux-gnu -static -O2 -g0 -fno-stack-protector \
  ...
  -c "$HERE/main.c" -o "$HERE/linux-misc.o"
