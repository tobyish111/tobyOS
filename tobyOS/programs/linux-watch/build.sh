#!/bin/bash
# Build the inotify + epoll acceptance test (linux-watch.elf).
#
...
#
# Usage:   bash programs/linux-watch/build.sh
# Output:  programs/linux-watch/linux-watch.elf   (gitignored; opt-in)
set -e

HERE="$(cd "$(dirname "$0")" && pwd)"
OUT="$HERE/linux-watch.elf"
...
clang --target=x86_64-unknown-linux-gnu -static -O2 -g0 -fno-stack-protector \
  ...
  -c "$HERE/main.c" -o "$HERE/linux-watch.o"
