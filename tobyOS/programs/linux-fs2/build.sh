#!/bin/bash
# Build the xattr + fd-identity + fexecve test (linux-fs2.elf).
#
...
#
# Usage:   bash programs/linux-fs2/build.sh
# Output:  programs/linux-fs2/linux-fs2.elf   (gitignored; opt-in)
set -e

HERE="$(cd "$(dirname "$0")" && pwd)"
OUT="$HERE/linux-fs2.elf"
...
clang --target=x86_64-unknown-linux-gnu -static -O2 -g0 -fno-stack-protector \
  ...
  -c "$HERE/main.c" -o "$HERE/linux-fs2.o"
