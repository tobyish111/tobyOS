#!/bin/bash
# Phase 3 slice 15: build the cgroup-v2 acceptance test (linux-cgroup.elf).
#
...
#
# Usage:   bash programs/linux-cgroup/build.sh
# Output:  programs/linux-cgroup/linux-cgroup.elf   (gitignored; opt-in)
set -e

HERE="$(cd "$(dirname "$0")" && pwd)"
OUT="$HERE/linux-cgroup.elf"
...
clang --target=x86_64-unknown-linux-gnu -static -O2 -g0 -fno-stack-protector \
  ...
  -c "$HERE/main.c" -o "$HERE/linux-cgroup.o"
