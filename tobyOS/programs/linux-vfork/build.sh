#!/bin/bash
# Build the xattr + fd-identity + fexecve test (linux-vfork.elf).
#
...
#
# Usage:   bash programs/linux-vfork/build.sh
# Output:  programs/linux-vfork/linux-vfork.elf   (gitignored; opt-in)
set -e

HERE="$(cd "$(dirname "$0")" && pwd)"
OUT="$HERE/linux-vfork.elf"
...
clang --target=x86_64-unknown-linux-gnu -static -O2 -g0 -fno-stack-protector \
  ...
  -c "$HERE/main.c" -o "$HERE/linux-vfork.o"
