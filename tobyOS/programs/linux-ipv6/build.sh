#!/bin/bash
# Build the AF_INET6 UDP + ::1 loopback test (linux-ipv6.elf).
#
# Static glibc, same Bootlin sysroot as the sibling linux-* tests.
#
# Usage:   bash programs/linux-ipv6/build.sh
# Output:  programs/linux-ipv6/linux-ipv6.elf   (gitignored; opt-in)
set -e

HERE="$(cd "$(dirname "$0")" && pwd)"
OUT="$HERE/linux-ipv6.elf"
...
clang --target=x86_64-unknown-linux-gnu -static -O2 -g0 -fno-stack-protector \
  ...
  -c "$HERE/main.c" -o "$HERE/linux-ipv6.o"
