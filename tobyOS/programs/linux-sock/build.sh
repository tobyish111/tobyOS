#!/bin/bash
# Build the socket-semantics test (linux-sock.elf).
#
...
#
# Usage:   bash programs/linux-sock/build.sh
# Output:  programs/linux-sock/linux-sock.elf   (gitignored; opt-in)
set -e

HERE="$(cd "$(dirname "$0")" && pwd)"
OUT="$HERE/linux-sock.elf"
...
clang --target=x86_64-unknown-linux-gnu -static -O2 -g0 -fno-stack-protector \
  ...
  -c "$HERE/main.c" -o "$HERE/linux-sock.o"
