#!/bin/bash
# Build the xattr + fd-identity + fexecve test (linux-cont.elf).
#
...
#
# Usage:   bash programs/linux-cont/build.sh
# Output:  programs/linux-cont/linux-cont.elf   (gitignored; opt-in)
set -e

HERE="$(cd "$(dirname "$0")" && pwd)"
OUT="$HERE/linux-cont.elf"
...
clang --target=x86_64-unknown-linux-gnu -static -O2 -g0 -fno-stack-protector \
  ...
  -c "$HERE/main.c" -o "$HERE/linux-cont.o"
