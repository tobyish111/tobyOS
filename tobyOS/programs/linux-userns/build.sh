#!/bin/bash
# Phase 3 slice 11: build the USER-namespace acceptance test (linux-userns.elf).
#
...
#
# Usage:   bash programs/linux-userns/build.sh
# Output:  programs/linux-userns/linux-userns.elf   (gitignored; opt-in)
set -e

HERE="$(cd "$(dirname "$0")" && pwd)"
OUT="$HERE/linux-userns.elf"
...
clang --target=x86_64-unknown-linux-gnu -static -O2 -g0 -fno-stack-protector \
  ...
  -c "$HERE/main.c" -o "$HERE/linux-userns.o"
