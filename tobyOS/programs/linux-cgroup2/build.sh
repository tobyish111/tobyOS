#!/bin/bash
# Phase 3 slice 16: build the memory+io controller acceptance test (linux-cgroup2.elf).
#
# Static glibc, same Bootlin sysroot as programs/linux-cred/build.sh. glibc
# rather than musl because the test drives unshare/setns/sethostname/shmget
...
#
# Usage:   bash programs/linux-cgroup2/build.sh
# Output:  programs/linux-cgroup2/linux-cgroup2.elf   (gitignored; opt-in)
set -e

HERE="$(cd "$(dirname "$0")" && pwd)"
OUT="$HERE/linux-cgroup2.elf"
