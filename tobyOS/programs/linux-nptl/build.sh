#!/bin/bash
# Build the NPTL thread-group acceptance test (linux-nptl.elf).
#
...
#
# Usage:   bash programs/linux-nptl/build.sh
# Output:  programs/linux-nptl/linux-nptl.elf   (gitignored; opt-in)
set -e

HERE="$(cd "$(dirname "$0")" && pwd)"
OUT="$HERE/linux-nptl.elf"
...
clang --target=x86_64-unknown-linux-gnu -static -O2 -g0 -fno-stack-protector \
  ...
  -c "$HERE/main.c" -o "$HERE/linux-nptl.o"
