#!/bin/bash
# Build the ptrace acceptance test (linux-ptrace.elf) -- a miniature strace.
#
# Static glibc, same Bootlin sysroot as the sibling linux-* tests. It needs
# <sys/ptrace.h> and <sys/user.h> for the request constants and
# user_regs_struct, so the register layout the kernel fills is checked against
# the SAME definition a real tracer would use rather than one written here.
#
# Usage:   bash programs/linux-ptrace/build.sh
# Output:  programs/linux-ptrace/linux-ptrace.elf   (gitignored; opt-in)
set -e

HERE="$(cd "$(dirname "$0")" && pwd)"
OUT="$HERE/linux-ptrace.elf"
...
clang --target=x86_64-unknown-linux-gnu -static -O2 -g0 -fno-stack-protector \
  ...
  -c "$HERE/main.c" -o "$HERE/linux-ptrace.o"
