#!/bin/bash
# Build the xattr + fd-identity + fexecve test (linux-thread2.elf).
#
# Static glibc, same Bootlin sysroot as the sibling linux-* tests. Static
# because bit5 fexecve-re-execs itself and the child must come up with no
# loader search path assumptions of its own.
#
# Usage:   bash programs/linux-thread2/build.sh
# Output:  programs/linux-thread2/linux-thread2.elf   (gitignored; opt-in)
set -e

HERE="$(cd "$(dirname "$0")" && pwd)"
OUT="$HERE/linux-thread2.elf"
