#!/bin/bash
# Phase 3 slice 9: build the MOUNT-namespace acceptance test (linux-mntns.elf).
#
# Static glibc, same Bootlin sysroot as programs/linux-cred/build.sh. glibc
# rather than musl because the test drives unshare(CLONE_NEWNS)/mount/pivot_root/propagation
...
#
# Usage:   bash programs/linux-mntns/build.sh
# Output:  programs/linux-mntns/linux-mntns.elf   (gitignored; opt-in)
set -e

HERE="$(cd "$(dirname "$0")" && pwd)"
OUT="$HERE/linux-mntns.elf"
...
clang --target=x86_64-unknown-linux-gnu -static -O2 -g0 -fno-stack-protector \
  ...
  -c "$HERE/main.c" -o "$HERE/linux-mntns.o"
