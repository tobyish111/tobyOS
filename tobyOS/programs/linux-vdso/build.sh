#!/bin/bash
# Build the xattr + fd-identity + fexecve test (linux-vdso.elf).
#
...
#
# Usage:   bash programs/linux-vdso/build.sh
# Output:  programs/linux-vdso/linux-vdso.elf   (gitignored; opt-in)
set -e

HERE="$(cd "$(dirname "$0")" && pwd)"
OUT="$HERE/linux-vdso.elf"
...
clang --target=x86_64-unknown-linux-gnu -static -O2 -g0 -fno-stack-protector \
  ...
  -c "$HERE/main.c" -o "$HERE/linux-vdso.o"
