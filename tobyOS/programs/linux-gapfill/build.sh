#!/bin/bash
# Build the post-Phase-3 gap-fill acceptance test (linux-gapfill.elf).
#
# Static glibc, same Bootlin sysroot as the sibling linux-* tests. Static so the
# bit2 chroot("/proc/self/fdinfo/") case works: after that chroot there is no
# filesystem left to resolve a loader from, which is the whole point of the
# directory Chromium picks.
#
# Usage:   bash programs/linux-gapfill/build.sh
# Output:  programs/linux-gapfill/linux-gapfill.elf   (gitignored; opt-in)
set -e

HERE="$(cd "$(dirname "$0")" && pwd)"
OUT="$HERE/linux-gapfill.elf"
