#!/bin/bash
# Build the xattr + fd-identity + fexecve test (linux-ldso.elf).
#
...
#
# Usage:   bash programs/linux-ldso/build.sh
# Output:  programs/linux-ldso/linux-ldso.elf   (gitignored; opt-in)
set -e

HERE="$(cd "$(dirname "$0")" && pwd)"
OUT="$HERE/linux-ldso.elf"
...
clang --target=x86_64-unknown-linux-gnu -static -O2 -g0 -fno-stack-protector \
  ...
  -c "$HERE/main.c" -o "$HERE/linux-ldso.o"
