#!/bin/bash
# Build the close-on-exec acceptance test (linux-cloexec.elf).
#
...
#
# Usage:   bash programs/linux-cloexec/build.sh
# Output:  programs/linux-cloexec/linux-cloexec.elf   (gitignored; opt-in)
set -e

HERE="$(cd "$(dirname "$0")" && pwd)"
OUT="$HERE/linux-cloexec.elf"
