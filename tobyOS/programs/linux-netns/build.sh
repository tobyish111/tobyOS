#!/bin/bash
# Phase 3 slice 12: build the NETWORK-namespace acceptance test (linux-netns.elf).
#
...
#
# Usage:   bash programs/linux-netns/build.sh
# Output:  programs/linux-netns/linux-netns.elf   (gitignored; opt-in)
set -e

HERE="$(cd "$(dirname "$0")" && pwd)"
OUT="$HERE/linux-netns.elf"
...
clang --target=x86_64-unknown-linux-gnu -static -O2 -g0 -fno-stack-protector \
  ...
  -c "$HERE/main.c" -o "$HERE/linux-netns.o"
