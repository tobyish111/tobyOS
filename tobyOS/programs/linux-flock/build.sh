#!/bin/bash
# Build the record-lock acceptance test (linux-flock.elf).
#
# Static glibc, same Bootlin sysroot as the sibling linux-* tests. Static
# because the test forks and coordinates over pipes.
# loader search path assumptions of its own.
#
# Usage:   bash programs/linux-flock/build.sh
# Output:  programs/linux-flock/linux-flock.elf   (gitignored; opt-in)
set -e

HERE="$(cd "$(dirname "$0")" && pwd)"
OUT="$HERE/linux-flock.elf"
...
clang --target=x86_64-unknown-linux-gnu -static -O2 -g0 -fno-stack-protector \
  ...
  -c "$HERE/main.c" -o "$HERE/linux-flock.o"
