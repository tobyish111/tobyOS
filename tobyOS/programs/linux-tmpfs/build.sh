#!/bin/bash
# Build the tmpfs acceptance test (linux-tmpfs.elf).
#
# Static glibc, same Bootlin sysroot as the sibling linux-* tests. It drives
# mount(2)/umount(2) directly rather than through busybox, so the assertions
# can be about VALUES -- bytes round-tripping, ENOSPC arriving at the size that
# was requested -- instead of an applet's exit status.
#
# Usage:   bash programs/linux-tmpfs/build.sh
# Output:  programs/linux-tmpfs/linux-tmpfs.elf   (gitignored; opt-in)
set -e

HERE="$(cd "$(dirname "$0")" && pwd)"
OUT="$HERE/linux-tmpfs.elf"
...
clang --target=x86_64-unknown-linux-gnu -static -O2 -g0 -fno-stack-protector \
  ...
  -c "$HERE/main.c" -o "$HERE/linux-tmpfs.o"
