#!/bin/bash
# Build the IPv6-vs-real-peer (SLIRP) test (linux-v6host.elf).
#
# Static glibc, same Bootlin sysroot as the sibling linux-* tests.
#
# Usage:   bash programs/linux-v6host/build.sh
# Output:  programs/linux-v6host/linux-v6host.elf   (gitignored; opt-in)
set -e

HERE="$(cd "$(dirname "$0")" && pwd)"
OUT="$HERE/linux-v6host.elf"
...
clang --target=x86_64-unknown-linux-gnu -static -O2 -g0 -fno-stack-protector \
  ...
  -c "$HERE/main.c" -o "$HERE/linux-v6host.o"
