#!/bin/bash
# Phase 3 slice 8: build the namespace acceptance test (linux-ns.elf).
#
...
#
# Usage:   bash programs/linux-ns/build.sh
# Output:  programs/linux-ns/linux-ns.elf   (gitignored; opt-in)
set -e

HERE="$(cd "$(dirname "$0")" && pwd)"
OUT="$HERE/linux-ns.elf"
...
clang --target=x86_64-unknown-linux-gnu -static -O2 -g0 -fno-stack-protector \
  ...
  -c "$HERE/main.c" -o "$HERE/linux-ns.o"
