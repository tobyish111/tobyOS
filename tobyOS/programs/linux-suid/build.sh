#!/bin/bash
# Linux slice 2: build the setuid-on-exec acceptance test (linux-suid.elf).
#
...
#
# Usage:   bash programs/linux-suid/build.sh
# Output:  programs/linux-suid/linux-suid.elf   (gitignored; opt-in)
set -e

HERE="$(cd "$(dirname "$0")" && pwd)"
OUT="$HERE/linux-suid.elf"
...
clang --target=x86_64-unknown-linux-gnu -static -O2 -g0 -fno-stack-protector \
  ...
  -c "$HERE/main.c" -o "$HERE/linux-suid.o"
