#!/bin/bash
# Build the xattr + fd-identity + fexecve test (linux-xattr.elf).
#
...
#
# Usage:   bash programs/linux-xattr/build.sh
# Output:  programs/linux-xattr/linux-xattr.elf   (gitignored; opt-in)
set -e

HERE="$(cd "$(dirname "$0")" && pwd)"
OUT="$HERE/linux-xattr.elf"
...
clang --target=x86_64-unknown-linux-gnu -static -O2 -g0 -fno-stack-protector \
  ...
  -c "$HERE/main.c" -o "$HERE/linux-xattr.o"
