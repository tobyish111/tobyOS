#!/bin/bash
# Build the xattr + fd-identity + fexecve test (linux-sysv.elf).
#
...
#
# Usage:   bash programs/linux-sysv/build.sh
# Output:  programs/linux-sysv/linux-sysv.elf   (gitignored; opt-in)
set -e

HERE="$(cd "$(dirname "$0")" && pwd)"
OUT="$HERE/linux-sysv.elf"
...
clang --target=x86_64-unknown-linux-gnu -static -O2 -g0 -fno-stack-protector \
  ...
  -c "$HERE/main.c" -o "$HERE/linux-sysv.o"
