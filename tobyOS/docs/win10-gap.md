...
2. **App compatibility (~36%)** — tobyOS now runs **unmodified Linux x86-64 binaries** broadly AND
   ...
   **trampoline** (it PUMPs one message at a time from the kernel and DispatchMessages each to `DialogProc`
   ...
   monospace fallback); screenshot of real Lato glyphs. **Windows: next** — `GetCommandLine`/registry stubs,
   per-thread TEBs, TTF for control labels + desktop, broader runtime shims. Still no .NET/UWP.
