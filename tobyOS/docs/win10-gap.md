...
2. **App compatibility (~16%)** — still no Win32/.NET/UWP. **Linux x86-64 binary compat
   ...
   (`busybox-dyn` `[LXDYN] PASS 7/7`). Next (B6): file-backed mmap (+ mmap-offset arg plumbing) so
   dynamic programs with a *separate* shared lib work. Still ecosystem-scale overall.
