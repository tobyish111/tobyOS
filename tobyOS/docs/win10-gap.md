...
2. **App compatibility (~36%)** — tobyOS now runs **unmodified Linux x86-64 binaries** broadly AND
   ...
   **trampoline** (it PUMPs one message at a time from the kernel and DispatchMessages each to `DialogProc`
   ...
   a box, picks "Premium", OKs, and the `DialogProc` reads them back → exit 14 (2 screenshots). **Windows:
   next** — real TTF fonts (C14b: kernel-side stb_truetype + a shipped font), `GetCommandLine`/registry stubs,
   per-thread TEBs, broader runtime shims. Still no .NET/UWP.
