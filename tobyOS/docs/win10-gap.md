...
2. **App compatibility (~17%)** — still no Win32/.NET/UWP. **Linux x86-64 binary compat
   (Track B, 2026-06-14):** a process ABI personality + Linux→tobyOS syscall translation runs
   unmodified Linux binaries — B1 static ELF, **B2 real musl-libc `busybox`**, **B3 directory listing**
   (`getdents64`), **B4 Linux signals** + cwd-`.` fix, **B5 DYNAMIC linking via the real musl `ld.so`**
   (`busybox-dyn` `[LXDYN] PASS 7/7`), **B6 file-backed mmap** (`[LXMMAP] PASS`). Next: an end-to-end
   multi-DSO dynamic-loading demo (needs a real musl multi-DSO binary; the file-backed-mmap primitive
   is done), SA_SIGINFO 3-arg handlers, and fork/exec/wait4 for `busybox sh`. Still ecosystem-scale.
