...
2. **App compatibility (~33%)** — tobyOS now runs **unmodified Linux x86-64 binaries** broadly AND
   ...
   **every** kernel-side shim into a fixed region of the shim page, so `GetProcAddress(hModule, "Func")`
   ...
   fd table since pids are reused). **Windows: next** — more GDI (pens/`LineTo`/bitmaps/`BitBlt`), more file
   ops (`SetFilePointer`/`FindFirstFile`), child/owned windows + dialogs, broader runtime shims as real
   binaries demand them. Still no .NET/UWP.
