...
- **2026-06-26** — **Track B milestone B10: unbranded Linux ELFs now DROP-AND-RUN — auto-detect the Linux ABI from `PT_INTERP`.**
  ...
  with zero preparation. **Honest scope:** a *static*, note-less `ELFOSABI_SYSV` binary carries no mark distinguishing
  it from a native static binary (both are OSABI 0, no `PT_INTERP`, no notes), so it cannot be safely auto-detected and
  still requires the brand; that's a fundamental ambiguity, documented rather than papered over. **Proof, clean under
