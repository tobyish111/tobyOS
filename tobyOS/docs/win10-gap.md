...
2. **App compatibility (~14%)** — still no Win32/.NET/UWP. **Linux x86-64 binary compat
   (Track B, 2026-06-14):** a process ABI personality + Linux→tobyOS syscall translation runs
   unmodified Linux binaries — B1 a hand-rolled static ELF, **B2 a real musl-libc `busybox`**, **B3
   directory listing** (`getdents64` → `busybox ls -la`), **B4 Linux signals** (`rt_sigaction`+restorer
   → `/bin/linux-sigtest` PASS) + cwd-`.` fix; 10-applet busybox battery `[LXBB] PASS`, incl. real
   file I/O + `struct stat`. Next: the dynamic loader (`ld.so`) for non-static binaries — the
   remaining big piece. Still ecosystem-scale overall.
