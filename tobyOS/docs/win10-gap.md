...
2. **App compatibility (~36%)** — tobyOS now runs **unmodified Linux x86-64 binaries** broadly AND
   ...
   **Windows: next** — `GetCommandLine`/registry stubs, common file dialogs (comdlg32), per-thread TEBs, TTF
   for control labels + desktop, broader runtime shims. Still no .NET/UWP.
