...
**Last updated:** 2026-08-13 (Chromium + the Linux container stack + page reclaim)

---

## Bottom line

**~34% overall Win10 feature parity** (canvas baseline was ~16%; ~26% at the last
update on 2026-06-08). The two months since are the largest jump this document has
recorded, and both halves of the project thesis moved.

**Track B (Linux) reached "runs the hard software".** Real, unmodified **Chromium 151**
runs end-to-end — multi-process, HTTPS, YouTube VP9 playback, in a TobyTK window
(~59.7 fps, 36 ms input latency, 138 ms navigation after the slice 68–120 perf arc).
Underneath it, the Linux-completeness arc landed **all six namespaces**, **seccomp-bpf**,
**cgroup v2** (`pids`/`cpu`/`memory`/`io`), **ptrace**, **tmpfs**, and a real **OCI
container runtime** running Alpine 3.19 under namespaces + `pivot_root` + cgroups + a uid
drop. The network stack grew a **netlink control plane** (`ip link/addr/route` work
against real busybox), **veth pairs**, a **learning bridge** and **NAT** — a container can
reach the internet.

**Track C (Win32/PE) advanced but remains the narrow side**, and this document should keep
saying so: 376 hand-written export shims (107 kernel32, 43 user32, 21 gdi32, ~150 CRT,
plus ws2_32/ole32/advapi32/comctl32). The yardstick is Wine, not Windows, and this is
roughly **1–2% of Wine's export surface**. There is also **no DLL loading from disk** —
an import outside the shim table binds to a null IAT slot and faults — so Track C scales
linearly with hand-written shims and cannot inherit a real DLL.

**What the number still holds back for.** Three things that read as features were found on
2026-08-13 to be initialized-but-never-called and are only now real: ASLR (banner only,
nothing randomized), the OOM killer (`oom_init` had no caller, so its own guard kept
`oom_check` inert), and page reclaim (`swap_out`'s only callers were a self-test). Reclaim
now works and is proven byte-exact, but only evicts from processes that are **not
executing** — the copy-then-unmap order is unsafe against a running writer, and fixing
that needs a "swap in progress" PTE state this kernel has no primitive for. **Real-hardware
validation is owed for everything since 2026-07**: every QEMU boot auto-logs-in as root,
so the whole stack is structurally blind to uid-dependent bugs. Power management (~9%) and
the GUI/desktop shell (~15%) remain the weakest subsystems and have not moved.
...
## Changelog
- **2026-08-13** — **Three subsystems that existed, printed a boot banner, and did nothing — plus the VMA-table leak found underneath them.**
  An audit asked which `src/*.c` files have no externally-referenced symbol except their `*_init`; that one query found three.
  **(1) ASLR was a print statement.** `aslr_init()` announced "ASLR enabled (stack, mmap, exe)" every boot and all five offset
  functions had ZERO callers — every process mapped at a fixed address. The mmap base is now drawn from `aslr_mmap_offset()` per
  address space (96 TiB region, ≤4 GiB offset, `align_for_len` re-applied on top so V8's 4 GiB cage alignment survives). Stack and
  exe bases are deliberately still fixed — the stack needs a per-process floor in the fault path, and the ET_DYN load bases are
  constants the Chromium arc is sensitive to — and `aslr.c` now documents them as unwired instead of naming them in a banner.
  **(2) The OOM killer never ran.** `oom_init()` had no caller, so `g_oom_initialized` stayed false — and `oom_check()`, which
  returns early on exactly that flag, had no caller either. Physical exhaustion had no recovery path at all: every allocator
  returned 0 and each caller failed its own way. Both now run from pid 0's idle loop at ~1 Hz. Called there rather than at the
  allocation failure (where `oom.h` says it belongs, and where Linux does it) because the demand-fault path runs
  `pmm_alloc_page()` **BKL-free** and `oom_kill()` enqueues onto the run queue — calling the scheduler from a BKL-free fault is
  the shape of race this kernel keeps hunting. Cost, stated in the code: recovery is reactive, so a burst between two checks still
  fails the allocation; what it fixes is sustained pressure. **(3) There was no page reclaim.** `swap_out()`'s only callers were
  zram's self-test, so no page was ever evicted and `page_fault.c` case 2.5 (swap-in) was unreachable in production.
  `mem_reclaim_pages()` now evicts private-anonymous pages, and lives **in mmap.c on purpose** so it reuses the audited
  shootdown-before-free batch, the unacked-shootdown quarantine, and the CoW-aware refcount handling verbatim rather than
  re-deriving them. Swap-in had to be made accounting-symmetric first: it allocates via `pmm_alloc_page()` outside the charge
  funnel, so without a matching charge every evict/fault-back cycle ratcheted `memory.current` permanently DOWN and a process
  could sit above `memory.max` forever by cycling through swap (`mm_user_page_charge_existing`). **Limits, deliberate:** reclaim
  only evicts from processes that are NOT EXECUTING — `swap_out` copies before the PTE flips, so a running writer's store would be
  silently lost; Linux unmaps and shoots down *first*, which needs a swap-in-progress PTE state this kernel has no primitive for.
  CoW-marked PTEs are skipped (the one present-page case the BKL-free fault path writes).
  **The leak underneath:** `g_vma_tables[]` had NO live reset path — `mmap_init_proc()` and `mmap_cleanup_proc()` both existed with
  zero callers. It is keyed by `proc_mm_pid()`, i.e. a RECYCLED `g_proc[]` slot index, so a new process inherited the dead one's
  mappings and `mmap_handle_page_fault` would fault a fresh zero page into an address it never mapped. Fixed on both sides: at
  reap (in the no-heir branch, after the barrier proves no CPU is still in the address space) and at execve (after
  `vfork_child_done` clears `mm_owner`, guarded so a thread or a live thread-group leader never wipes its group's table, because
  execve here does not implement Linux's "exec kills the other threads").
  **Proof, not assertion:** `logs/reclaim.sh` (`-DRECLAIM_BOOT`) fills 256 anonymous pages with a pattern keyed on BOTH page index
  and byte offset, sleeps so the process is descheduled, then verifies every byte — 3 rounds, re-dirtied between each. A pass
  additionally requires `evicted > 0` AND `faulted_back > 0`, because a run that evicted nothing would exit 0 having tested
  nothing. It earned that: the first run exited 0 with **zero pages evicted** (correctly reported FAIL), and the second panicked
  the kernel with a #GP — `*pte & PAGE_MASK` keeps bit 63 (NX) and the software bits, so the "physical address" handed to
  `swap_out` was non-canonical; the fix is `PTE_ADDR_MASK`. Final: **1537 evicted, 1281 faulted back, 3×256 pages byte-exact,
  0 faults.** `lxposix --full` 23/23 and `linux-cgroup2` **0xff** (bit6: `memory.current` equals RSS exactly; bit7: no drift over
  four fork/allocate/exit cycles) both still green, which is what certifies the accounting change.
  **Also:** ramfs stopped fabricating a mode for every file (hardcoded 00444/00555 at all three reporting sites, USTAR header
  ignored) — `ls -l /bin` went from `-r--r--r--` to `-rwxr-xr-x 0 0`; this required fixing the initrd tar too, because `chmod` is a
  **silent no-op** on the MSYS build host, so modes are now forced at tar time in two non-overlapping passes (verified: identical
  991-entry file set before and after, only modes changed). Hard links and a real `nlink` added. And the build finally tracks
  header dependencies (`-MMD -MP`): kernel objects depended on `include/tobyos/*.h`, ONE directory level, so `abi/abi.h` — the
  whole Linux ABI — was invisible, and `libtoby/`/`programs/` had no header prerequisites at all. That was the mechanical cause of
  two hand-carried rules ("struct proc grows ⇒ make clean", "struct tk_window grew ⇒ touch all program sources").
  Memory ~26% → ~34%, Security ~19% → ~30%, Filesystem ~31% → ~36%.
- **2026-07 → 2026-08-12** — **The two arcs this document had not yet scored: Chromium, and Linux completeness.**
  Recorded here as a summary; the per-slice history lives in `docs/chromium-*.md` and `docs/linux-arc-handoff-phase3.md`.
  **Chromium.** Real, unmodified **Chromium 151** runs end-to-end on tobyOS — multi-process, HTTPS, YouTube **VP9** playback, in a
  TobyTK window via `chromewin` + CDP, pinned on the taskbar and launchable by a click (verified by making the guest click its own
  taskbar). The slice 68–120 perf arc took it to ~59.7 fps, 36 ms input latency and 138 ms navigation; Tier 3 (GPU) is CLOSED.
  The blocker that gated it for weeks was **`clone(2)` ignoring `child_stack`** — Chromium calls glibc's clone() *library*
  function, whose child pops its entry function off the very stack tobyOS was discarding. Four bugs in one chain; the old test
  passed because the RAW syscall form is the single case where ignoring `child_stack` is correct.
  **Linux completeness (slices 8–16).** All six namespaces (UTS, IPC, mount + real propagation + `pivot_root`, PID with vpid
  translation, USER, NET), **seccomp-bpf**, **cgroup v2** with `pids`/`cpu`/`memory`/`io`, **ptrace** (syscall stops, PEEK/POKE,
  GETREGS — so `strace` is possible), **tmpfs**, and the capstone: a real **OCI runtime** running Alpine 3.19 under all six
  namespaces + `pivot_root` + cgroup v2 + seccomp + a uid drop. The networking half went from nothing to a **netlink control
  plane** driven by real busybox `ip`, **veth**, a learning **bridge** and **NAT** — a container reaches the internet.
  The arc's method note, worth keeping: its three best findings were all a GREEN test with a WRONG NUMBER — ARP resolving to the
  host NIC's MAC (the ARP *payload* carries its own sender-MAC, a second layer beyond `eth_send`), `chown` refusing with `ENOMEM`
  (raw VFS codes leaking as unrelated errnos), and `memory.current` matching by accident. Assert the value, not the exit code.
  App Compatibility ~53% → ~62%, Networking ~32% → ~42%, Audio/Media ~15% → ~30%.
...
- **2026-06-27** — **Track B (Linux input + graphics, INTERACTIVE): a single dynamically-linked musl Linux app reads the MOUSE via `/dev/input/event1` (evdev) and PAINTS to `/dev/fb0` — `[PAINT] VERDICT: PASS exit=0 strokes=ok`.**
  This closes the loop: a Linux GUI app now consumes real pointer input *and* renders, in one event loop. The evdev layer was generalised to **two minors** — `event0` = keyboard (unchanged), **`event1` = mouse** — each with its own ring, identity (`EVIOCGNAME` → `"tobyOS PS/2 Mouse"`), and capabilities (`EVIOCGBIT(0)` advertises `EV_SYN|EV_KEY|EV_REL`; `EVIOCGBIT(EV_REL)` → `REL_X|REL_Y`; `EVIOCGBIT(EV_KEY)` → `BTN_LEFT/RIGHT/MIDDLE`). The PS/2 mouse driver now mirrors every report into the device via the new `evdev_feed_mouse()` (called from `mouse_inject_event`), emitting `EV_REL` motion + `BTN_*` press/release edges + a `SYN_REPORT` frame — the genuine PS/2-mouse event shape. The app `programs/linux-paint` is a **dynamic musl PIE** (real ld-musl): it opens `/dev/fb0` + `/dev/input/event1`, accumulates `EV_REL` deltas into a cursor, and on each `BTN_LEFT` press stamps a coloured square — "paint where you click" — presenting each stroke with `FBIOPAN_DISPLAY`. The `#ifdef PAINT_BOOT` harness injects three mouse moves + left-clicks exactly as the PS/2 driver would (`mouse_inject_event` → `evdev_feed_mouse`) and the app paints at (300,300)/(700,400)/(1000,560). **Proof, clean under `+smep,+smap`, 0 faults:** `paint: mouse 'tobyOS PS/2 Mouse'`, `paint: click 1 at (300,300)…`, and the kernel sampled the strokes off the scanout — `R=ff3030 G=30ff30 B=3060ff` (`logs/paint.sh`, `logs/paint.png`). **No regression:** the always-compiled changes are additive (the evdev multi-device refactor + the one `evdev_feed_mouse()` call in the mouse path + the `event1` open route + a `file_clone` minor case); the keyboard `event0` proof still passes (`[EVDEV] VERDICT: PASS`). Harness gated by `#ifdef PAINT_BOOT`; default ISO still builds clean.
- **2026-06-27** — **Track B (Linux graphics, OFF-THE-SHELF): a REAL, UNMODIFIED third-party graphical Linux binary draws to `/dev/fb0` — the prebuilt Alpine busybox `fbsplash` applet — `[OTSGFX] VERDICT: PASS fbset=0 fbsplash=0 markers=ok`.**
  The earlier framebuffer proofs (`linux-fb`, `linux-fbgui`) were *first-party* apps we wrote. This runs a genuine binary **we did not write**: the `fbsplash` applet of the prebuilt Alpine **musl** busybox (`programs/busybox/busybox-dyn`, a dynamic PIE loaded through the real `ld-musl`). It `open()`s `/dev/fb0`, queries geometry with `FBIOGET_*SCREENINFO`, `mmap()`s the scanout, parses a binary **P6 PPM** (`/etc/splash.ppm`, generated by the host tool `programs/fbsplash/mkppm.c`), and writes the pixels straight into the mapping. Two upstream gaps had to be closed to run it unmodified: (1) `fbsplash` (like most fb programs) draws into the mmap and **exits without `FBIOPAN_DISPLAY`** — on real hardware the mmap *is* the live scanout — so the kernel now **presents the fbdev shadow on `proc_exit`** (the dying process's pages are still mapped, the final blit lands, and the hardware scanout retains it); (2) the Alpine `fbsplash` opens a **console device** to put the VT into graphics mode, so `/dev/console`, `/dev/tty`, and `/dev/tty<N>` are now openable (backed by the kernel console) and the **KD/VT ioctls** (`0x4B**`/`0x56**`, e.g. `KDSETMODE`/`KDGKBTYPE`) are accepted as no-ops. As a read-only companion check the harness also runs the off-the-shelf busybox **`fbset`**, which prints our real geometry via the same ioctls: `mode "1280x800-0"`, `geometry 1280 800 1280 800 32`, exit 0. **Proof, clean under `+smep,+smap`, 0 faults:** the kernel samples the three RGB marker squares baked into the PPM straight off the gfx scanout — `marker pixels R=ff0000 G=00ff00 B=0000ff` — and the screenshot shows the gradient + markers (`logs/otsgfx.sh`, `logs/otsgfx.png`). **No regression:** the always-compiled changes are additive (present-on-exit hook + console/tty device opens + KD/VT ioctl no-ops); the harness is gated by `#ifdef OTSGFX_BOOT`. Default ISO still builds clean.
- **2026-06-27** — **Track B (Linux input): `/dev/input/event0` LINUX EVDEV device — a dynamically-linked musl Linux app reads the keyboard through the genuine evdev ABI — `[EVDEV] VERDICT: PASS exit=0` (decoded "TOBY").**
  The framebuffer milestones gave Linux apps an *output* surface; this gives them the matching *input* surface — the Linux input layer. A new `FILE_KIND_EVDEV` device, `/dev/input/event0`, delivers the standard evdev stream: `read()` returns fixed 24-byte `struct input_event` records (`EV_KEY` make/break + `EV_SYN` frames) from a small kernel ring, blocking (yield) on empty unless `O_NONBLOCK`; the `EVIOCG*` ioctls report identity + capabilities (`EVIOCGVERSION` → `1.0.1`, `EVIOCGID`, `EVIOCGNAME` → `"tobyOS PS/2 Keyboard"`, `EVIOCGBIT(0)`/`EVIOCGBIT(EV_KEY)` capability bitmasks). The PS/2 keyboard driver feeds raw make/break into the ring via the new `evdev_feed_key()` — and because PS/2 scancode **set 1** was the source of the Linux keycode table, the keycode *equals* the scancode across the main block (KEY_ESC=1=0x01 … KEY_SPACE=57=0x39), so the mapping is the identity. The reader is `programs/linux-evdev`, a **dynamic musl PIE** (same `crt.c`/`build.sh`/`ld-musl` path as `linux-fbgui`): it `open()`s the device, queries identity with `EVIOCGVERSION`/`EVIOCGNAME`, and `read()`s `input_event` records via real libc, collecting `EV_KEY` *press* events until ENTER. The `#ifdef EVDEV_BOOT` harness injects the keycodes for **T, O, B, Y, ENTER** exactly as the PS/2 driver would, and the app exits 0 only if it decoded the string `"TOBY"`. **Proof, clean under `+smep,+smap`, 0 faults:** `[proc] loaded interpreter '/lib/ld-musl-x86_64.so.1'`, `evdev: 'tobyOS PS/2 Keyboard' driver v1.0.1`, `evdev: decoded "TOBY" from 4 EV_KEY press events`, `[EVDEV] VERDICT: PASS exit=0` (`logs/evdev.sh`). **No regression:** the always-compiled changes are additive (the `FILE_KIND_EVDEV` device + its `evdev_read`/`evdev_ioctl` + the one `evdev_feed_key()` call in the keyboard ISR path + `ABI_EAGAIN`/`ABI_O_NONBLOCK`); the harness is gated by `#ifdef EVDEV_BOOT` (undefined in default/regression builds). The default ISO still builds clean.
- **2026-06-27** — **Track B (Linux graphics, DYNAMIC): a DYNAMICALLY-LINKED musl Linux app draws to `/dev/fb0` through the real `ld-musl` loader + libc — `[FBGUI] VERDICT: PASS exit=0 markers=ok`.**
  The static raw-syscall fbdev proof (`linux-fb`) showed the device worked; this proves the **real-world** Linux graphics path that an actual program uses: not a hand-rolled `syscall` instruction, but **libc**. `programs/linux-fbgui` is a musl **PIE** (`PT_INTERP=/lib/ld-musl-x86_64.so.1`, `DT_NEEDED libc.musl-x86_64.so.1` — the *same* `ld-musl` loader `busybox-dyn`/CPython use), built in-tree by `programs/linux-fbgui/build.sh`: the freestanding clang has no musl sysroot, so it links **directly against the real musl `libc.so`** that ships in the initrd (a faithful `crt.c` supplies the standard `_start → __libc_start_main` entry, so no `Scrt1.o` is needed) and is brandelf'd `EI_OSABI=3`. At boot the kernel loads the PIE **and** `ld-musl`, the loader self-relocates and resolves the libc symbols, and the app draws via **genuine libc calls** — `open()`/`ioctl()`/`mmap()` syscall wrappers, the stdio `printf`/`fprintf` family (it prints `fbgui: 1280x800 32bpp line=5120 id='tobyos-fb' (dynamic musl libc via ld-musl)` to the console), and the `malloc` heap — using the verbatim `<linux/fb.h>` UAPI (`FBIOGET_VSCREENINFO`/`FSCREENINFO`, `FBIOPAN_DISPLAY`). It paints classic 8-bar SMPTE-style colour bars + three solid RGB marker squares and presents three frames; the kernel then samples the marker centres straight off the gfx scanout, so a PASS proves the **libc-drawn** pixels reached the display, not merely that the loader ran. **Proof, clean under `+smep,+smap`, 0 faults:** `[proc] pid 1 'linux-fbgui' -> Linux ABI personality (EI_OSABI=3 interp=/lib/ld-musl-x86_64.so.1)`, `[proc] loaded interpreter '/lib/ld-musl-x86_64.so.1'`, `FBGUI: app exit=0; marker pixels R=ff0000 G=00ff00 B=0000ff`, `[FBGUI] VERDICT: PASS exit=0 markers=ok` (`logs/fbgui.sh` + screenshot `logs/fbgui.png` showing the colour bars + RGB squares). **No regression:** the only always-compiled change is the opt-in initrd staging of `linux-fbgui` + the `ld-musl` loader (skipped when absent); the harness is gated by `#ifdef FBGUI_BOOT` (undefined in default/regression builds). *(Build note: `build.sh` sets `MSYS2_ARG_CONV_EXCL=*` so MSYS2 doesn't rewrite the absolute `--dynamic-linker=/lib/...` path into a Windows path and bake a bogus `PT_INTERP`.)*
- **2026-06-27** — **Track B (Linux graphics): `/dev/fb0` LINUX FRAMEBUFFER device — a genuine Linux fbdev app mmaps the scanout and draws real pixels — `[FBDEV] VERDICT: PASS exit=0 markers=ok`.**
  The Linux ABI surface could run console/ncurses apps but had **no graphics path**: a real Linux GUI program that talks to the kernel directly opens `/dev/fb0`, queries geometry, `mmap`s the scanout and writes pixels — and that device simply didn't exist. This adds a **Linux framebuffer device** (`FILE_KIND_FB`): `sys_open("/dev/fb0")` creates an fb file and initialises `g_fbdev` from the live `gfx` geometry (width/height/bpp/line-length); `LX_ioctl` services the real fbdev ioctls — `FBIOGET_VSCREENINFO` (fills a `struct fb_var_screeninfo`: xres/yres, bits_per_pixel=32, RGBA bitfields), `FBIOGET_FSCREENINFO` (`fb_fix_screeninfo`: line_length, smem_len), `FBIOPUT_VSCREENINFO` (accept-matching), `FBIOPAN_DISPLAY` and `FBIO_WAITFORVSYNC` (present); and `LX_mmap` on the fb fd returns a **demand-paged anonymous shadow region** sized to the scanout. Rather than handing the app raw hardware frames (which `vmm_destroy_user_pml4`/`munmap` would `pmm_free_page` on teardown — freeing real device memory), the app draws into its shadow buffer and a present ioctl **copies it into `gfx_backbuf` + `gfx_flip()`** (standard fbdev double-buffering). A first-party `linux-fb` (genuine x86-64 **static ELF, raw syscalls, no libc** — would run unmodified on Linux) opens `/dev/fb0`, reads both screeninfos, mmaps the scanout, draws a blue→teal **gradient** plus three solid **RGB marker squares**, and presents with `FBIOPAN_DISPLAY`. It runs **before** `m14_init()` (no compositor competing for the framebuffer); **the kernel then samples the three marker centres straight off the scanout** so a PASS proves the pixels actually reached the display, not merely that the syscalls returned. **Proof, clean under `+smep,+smap`, 0 faults:** `FBDEV: app exit=0; marker pixels R=ff0000 G=00ff00 B=0000ff`, `[FBDEV] VERDICT: PASS exit=0 markers=ok` (`logs/fbdev.sh` + screenshot `logs/fbdev.png` showing the gradient + RGB squares). **No regression:** the always-compiled changes are additive (`FILE_KIND_FB`, the `/dev/fb0` open case, and fb routing inside `LX_ioctl`/`LX_mmap` guarded by the fd kind); the harness is gated by `#ifdef FBDEV_BOOT` (undefined in default/regression builds).
- **2026-06-27** — **Track C milestone C25: comctl32 COMMON CONTROLS — a report-view ListView, a TreeView, a multi-part StatusBar, a Progress bar and a Toolbar — `[WINPE25] VERDICT: PASS exit=25`.**
  Through C24 the GUI surface covered user32's built-in controls (BUTTON/EDIT/STATIC/CHECKBOX/LISTBOX/SCROLLBAR/COMBOBOX/RADIO/GROUPBOX + a kernel-reimplemented TAB) but **`comctl32.dll` had zero exports** — the controls that make a real Win32 app look like one (ListView, TreeView, StatusBar, Progress, Toolbar) were entirely missing. This adds a **comctl32 common-controls layer**: `InitCommonControlsEx` is acknowledged and the five common-control window classes (`SysListView32`, `SysTreeView32`, `msctls_statusbar32`, `msctls_progress32`, `ToolbarWindow32`) are recognised by `CreateWindowExA` as virtual children backed by a new rich per-control store (`g_win32_cc[]`: a ListView column/row/subitem grid, a TreeView node array with parent/level/expand, multi-part StatusBar panes, Progress range/pos, Toolbar button count). The kernel **is** each control's WndProc: a dedicated handler services `LVM_INSERTCOLUMN/INSERTITEM/SETITEMTEXT/GETITEMTEXT/GETITEMCOUNT/SETITEMSTATE/GETNEXTITEM`, `TVM_INSERTITEM/GETCOUNT/EXPAND/SELECTITEM/GETNEXTITEM`, `SB_SETPARTS/SETTEXT/GETTEXT`, `PBM_SETRANGE32/SETPOS/SETSTEP/STEPIT/GETPOS`, and `TB_ADDBUTTONS/BUTTONCOUNT` (dispatched by control **type** first, since the StatusBar/Progress/Toolbar message numbers all alias `WM_USER+n`), and each control renders into its parent's client area (report grid with column headers + selection highlight, indented collapsible tree, bottom-docked status panes, filled progress bar, button strip). A first-party `win-cc25.exe` (stock clang mingw, links `-lcomctl32`) builds a 3-column/4-row ListView with subitems, a Project→src/include/docs→main.c tree, a 2-part status bar, a 0–100 progress bar, and a 3-button toolbar **purely through the comctl32 API**, then **reads every value back** (counts, item + subitem text, selection, tree caret, progress pos after `STEPIT`, status text, button count) — exiting 25 only if all round-trips match, so a PASS proves the control state lives in the kernel, not that the binary merely launched. **Proof, clean under `+smep,+smap`, 0 faults:** `[WINPE25] VERDICT: PASS exit=25` (`logs/c25.sh` + screenshot `logs/c25.png`). **No regression:** the C14 control battery (`win-ctrl14` combobox/radio/tab + `win-dlg14` resource dialog) still PASSes (`WINPE14`/`WINPE14D`); the only always-compiled changes are additive (new control kinds, the `g_win32_cc[]` store, the comctl32 message handler gated by `win32_is_comctl()`, and the six comctl32 shim entries), and the harness is gated by `#ifdef WINPE25_BOOT`.
