...
2. **App compatibility (~12%)** — still no Win32/.NET/UWP. **Linux x86-64 binary compat
   ...
   directory listing** (`getdents64` → `busybox ls -la` works); 9-applet battery `[LXBB] PASS`,
   incl. real file I/O + `struct stat` translation. Next: Linux signals, then the dynamic loader
   (`ld.so`). Still ecosystem-scale overall.
