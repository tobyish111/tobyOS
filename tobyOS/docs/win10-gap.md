...
2. **App compatibility (~32%)** — tobyOS now runs **unmodified Linux x86-64 binaries** broadly AND
   ...
   **every** kernel-side shim into a fixed region of the shim page, so `GetProcAddress(hModule, "Func")`
   ...
   `GetProcAddress(…, "GetMessageA")` works cross-DLL — exit 9 (`[WINPE9] PASS`). **Windows: next** —
   multi-window, more GDI (pens/lines/bitmaps/`BitBlt`), more file ops (`SetFilePointer`/`FindFirstFile`),
