...
2. **App compatibility (~21%)** — still no Win32/.NET/UWP. **Linux x86-64 binary compat
   ...
   **B4 Linux signals**, **B5 DYNAMIC linking via the real musl `ld.so`**, **B6 file-backed mmap**,
   **B7 end-to-end multi-DSO** (Alpine `file` loads `libmagic.so.1`), **B8 a REAL shell** (`busybox sh`
   runs external commands via fork/execve/wait4, `[LXSH] PASS`). Fixed two latent kernel bugs along the
   way (PAGE_MASK >4 GiB truncation; execve user-stack pack missing a SMAP window). Next: threads
   (`clone`), then the long tail of syscall breadth (diminishing returns). The loader/libc/process surface
   is now solid.
