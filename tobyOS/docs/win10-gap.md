...
2. **App compatibility (~31%)** — tobyOS now runs **unmodified Linux x86-64 binaries** broadly AND
   ...
   **screenshot of the real green Win32 window on the tobyOS desktop**, cursor on it, plus the serial log
   (`GetMessage -> WM_0201`…`WM_CLOSE`…`WM_DESTROY`) and exit 8. **Windows: next** — `GetModuleHandle`/
   `GetProcAddress`, multi-window, more GDI (brushes/pens/lines/bitmaps), a real desktop launcher menu entry.
   Still no .NET/UWP.
