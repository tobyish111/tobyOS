...
2. **App compatibility (~36%)** — tobyOS now runs **unmodified Linux x86-64 binaries** broadly AND
   ...
   **`SendMessageA`** (the universal control channel: `BM_GETCHECK`/`SETCHECK`, `LB_ADDSTRING`/`GETCURSEL`/
   ...
   sizes/colours + a centred `DrawText` (exit 13, screenshot of the fonts). **Windows: next** — dialogs from
   resource templates (`DialogBox`/`DLGTEMPLATE`), more controls (COMBOBOX/radio/tab), real TTF fonts,
   `GetCommandLine`/registry stubs, broader runtime shims. Still no .NET/UWP.
