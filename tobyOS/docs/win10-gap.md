...
2. **App compatibility (~19%)** — still no Win32/.NET/UWP. **Linux x86-64 binary compat
   ...
   **B6 file-backed mmap**, **B7 end-to-end multi-DSO** (Alpine `file` loads `libmagic.so.1` via ld-musl;
   `[LXMULTI] PASS` — also fixed a real PAGE_MASK >4 GiB truncation bug). Next: `fork`/`exec`/`wait4`
   (→ `busybox sh`), threads (`clone`), SA_SIGINFO. Still ecosystem-scale, but the loader + libc surface
   are now solid.
