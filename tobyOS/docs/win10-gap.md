...
2. **App compatibility (~34%)** — tobyOS now runs **unmodified Linux x86-64 binaries** broadly AND
   ...
   **Child/owned windows:** `CreateWindowExA` now reads `hWndParent`, records the parent link, and destroying
   ...
   WM_DESTROY (exit 11), proven by a screenshot of the GDI window + its child. **Windows: next** — bitmaps/
   `BitBlt`, more controls (BUTTON/EDIT), dialogs, `GetCommandLine`/registry stubs, broader runtime shims as
   real binaries demand them. Still no .NET/UWP.
